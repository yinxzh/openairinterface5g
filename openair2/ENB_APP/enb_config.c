  ...
  if ( RUParamList.numelt > 0) {
    ...
    for (j = 0; j < RC.nb_RU; j++) {
      ...
      if (RC.nb_L1_inst >0)
	    ...
	    RC.ru[j]->num_eNB                           = 0;
      for (int i=0;i<RC.ru[j]->num_eNB;i++) RC.ru[j]->eNB_list[i] = RC.eNB[RUParamList.paramarray[j][RU_ENB_LIST_IDX].iptr[i]][0];     
	...
	for (int i=0;i<RC.ru[j]->num_bands;i++) RC.ru[j]->band[i] = RUParamList.paramarray[j][RU_BAND_LIST_IDX].iptr[i]; 
	...
	RC.ru[j]->att_rx                         = *(RUParamList.paramarray[j][RU_ATT_TX_IDX].uptr); 
      ...
      if (RUParamList.paramarray[j][RU_NBIOTRRC_LIST_IDX].numelt > 0 && RC.nb_nb_iot_rrc_inst > 0) {
          ...
          if (RC.ru[j]->NbIoT->NB_IoT_eNB_list != NULL) {
              RC.ru[j]->NbIoT->num_NbIoT = RUParamList.paramarray[j][RU_NBIOTRRC_LIST_IDX].numelt;
