#ifndef RRC_ENB_S1AP_NB_IoT_H_
#define RRC_ENB_S1AP_NB_IoT_H_
...
typedef struct rrc_ue_s1ap_ids_NB_IoT_s {
  /* Tree related data */
  RB_ENTRY(rrc_ue_s1ap_ids_NB_IoT_s) entries;
  ...
  // value
  rnti_t   ue_rnti;
} rrc_ue_s1ap_ids_NB_IoT_t;

int
rrc_eNB_S1AP_compare_ue_ids_NB_IoT(
  struct rrc_ue_s1ap_ids_NB_IoT_s* c1_pP,
  struct rrc_ue_s1ap_ids_NB_IoT_s* c2_pP
);

struct rrc_rnti_tree_NB_IoT_s;

RB_PROTOTYPE(rrc_rnti_tree_NB_IoT_s, rrc_ue_s1ap_ids_NB_IoT_s, entries, rrc_eNB_S1AP_compare_ue_ids_NB_IoT);
 ...
 */
int rrc_eNB_process_S1AP_DOWNLINK_NAS_NB_IoT(MessageDef *msg_p, const char *msg_name, instance_t instance, mui_t *rrc_eNB_mui);
 ...
 *\return 0 when successful, -1 if the UE index can not be retrieved.
 */
int rrc_eNB_send_S1AP_E_RAB_SETUP_RESP_NB_IoT(const protocol_ctxt_t* const ctxt_pP, rrc_eNB_ue_context_NB_IoT_t*  const ue_context_pP, uint8_t xid );
