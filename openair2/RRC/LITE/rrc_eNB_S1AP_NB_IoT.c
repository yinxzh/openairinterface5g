...
# if defined(ENABLE_ITTI)
//------------------------------------------------------------------------------
struct rrc_ue_s1ap_ids_NB_IoT_s*
rrc_eNB_S1AP_get_ue_ids_NB_IoT(
...
//------------------------------------------------------------------------------
{
  rrc_ue_s1ap_ids_NB_IoT_t *result = NULL;
  rrc_ue_s1ap_ids_NB_IoT_t *result2 = NULL;
  hashtable_rc_t     h_rc;

  // we assume that a rrc_ue_s1ap_ids_NB_IoT_s is initially inserted in initial_id2_s1ap_ids
  if (eNB_ue_s1ap_id > 0) {
		  ...
		  if (NULL != result2) {
		    *result2 = *result;
            ...
            h_rc = hashtable_insert(rrc_instance_pP->s1ap_id2_s1ap_ids,
		    		               ...
		    		               result2);
            if (h_rc != HASH_TABLE_OK) {
              LOG_E(S1AP, "[NB_IoT][eNB %ld] Error while hashtable_insert in s1ap_id2_s1ap_ids eNB_ue_s1ap_id %"PRIu32"\n",
		    		  rrc_instance_pP - RC.nb_iot_rrc[0], eNB_ue_s1ap_id);
            }
		  }
		}
	  }
...
//------------------------------------------------------------------------------
void
rrc_eNB_S1AP_remove_ue_ids_NB_IoT(
  eNB_RRC_INST_NB_IoT*              const rrc_instance_pP,
  struct rrc_ue_s1ap_ids_NB_IoT_s* const ue_ids_pP
)
//------------------------------------------------------------------------------
{
  ...
  if (rrc_instance_pP == NULL) {
    LOG_E(RRC, "[NB_IoT]Bad RRC instance\n");
    return;
  }

  if (ue_ids_pP == NULL) {
    LOG_E(RRC, "[NB_IoT]Trying to free a NULL S1AP UE IDs\n");
    return;
  }
  ...
  if (eNB_ue_s1ap_id > 0) {
	...
	if (h_rc != HASH_TABLE_OK) {
	  LOG_W(RRC, "[NB_IoT]S1AP Did not find entry in hashtable s1ap_id2_s1ap_ids for eNB_ue_s1ap_id %u\n", eNB_ue_s1ap_id);
	} else {
	  LOG_W(RRC, "[NB_IoT]S1AP removed entry in hashtable s1ap_id2_s1ap_ids for eNB_ue_s1ap_id %u\n", eNB_ue_s1ap_id);
	}
  }
  ...
  if (ue_initial_id != UE_INITIAL_ID_INVALID) {
	...
	if (h_rc != HASH_TABLE_OK) {
	  LOG_W(RRC, "[NB_IoT]S1AP Did not find entry in hashtable initial_id2_s1ap_ids for ue_initial_id %u\n", ue_initial_id);
	} else {
	  LOG_W(RRC, "[NB_IoT]S1AP removed entry in hashtable initial_id2_s1ap_ids for ue_initial_id %u\n", ue_initial_id);
	}
  }
}
...
rrc_eNB_get_ue_context_from_s1ap_ids_NB_IoT(
  const instance_t  instanceP,
  const uint16_t    ue_initial_idP,
  const uint32_t    eNB_ue_s1ap_idP
)
{
  rrc_ue_s1ap_ids_NB_IoT_t* temp = NULL;
  temp =
    rrc_eNB_S1AP_get_ue_ids_NB_IoT(
      RC.nb_iot_rrc[ENB_INSTANCE_TO_MODULE_ID(instanceP)],
      ue_initial_idP,
      eNB_ue_s1ap_idP);

  if (temp) {

    return rrc_eNB_get_ue_context_NB_IoT(
             RC.nb_iot_rrc[ENB_INSTANCE_TO_MODULE_ID(instanceP)],
             temp->ue_rnti);
  }
 ...
 */
static CipheringAlgorithm_r12_t rrc_eNB_select_ciphering_NB_IoT(uint16_t algorithms)
{
 ...
 *\return the selected algorithm.
 */
static e_SecurityAlgorithmConfig__integrityProtAlgorithm rrc_eNB_select_integrity_NB_IoT(uint16_t algorithms)
{
 ...
 */
static int
rrc_eNB_process_security_NB_IoT(
  const protocol_ctxt_t* const ctxt_pP,
  rrc_eNB_ue_context_NB_IoT_t*          const ue_context_pP,
...
)
{
  ...
  // translation
  LOG_D(RRC,
        "[NB_IoT][eNB %d] NAS security_capabilities.encryption_algorithms %u AS ciphering_algorithm %lu NAS security_capabilities.integrity_algorithms %u AS integrity_algorithm %u\n",
        ctxt_pP->module_id,
  ...
  cipheringAlgorithm = rrc_eNB_select_ciphering_NB_IoT (ue_context_pP->ue_context.security_capabilities.encryption_algorithms);
  ...
  integrityProtAlgorithm = rrc_eNB_select_integrity_NB_IoT (ue_context_pP->ue_context.security_capabilities.integrity_algorithms);
  ...
  LOG_I (RRC, "[NB_IoT][eNB %d][UE %x] Selected security algorithms (%p): %lx, %x, %s\n",
         ctxt_pP->module_id,
 ...
 */
//------------------------------------------------------------------------------
static void process_eNB_security_key_NB_IoT (
  const protocol_ctxt_t* const ctxt_pP,
  rrc_eNB_ue_context_NB_IoT_t*          const ue_context_pP,
...
//------------------------------------------------------------------------------
static void
rrc_pdcp_config_security_NB_IoT(
  const protocol_ctxt_t* const ctxt_pP,
  rrc_eNB_ue_context_NB_IoT_t*          const ue_context_pP,
...
//------------------------------------------------------------------------------
void
rrc_eNB_send_S1AP_INITIAL_CONTEXT_SETUP_RESP_NB_IoT(
  const protocol_ctxt_t* const ctxt_pP,
  rrc_eNB_ue_context_NB_IoT_t*          const ue_context_pP
)
//------------------------------------------------------------------------------
{
  ...
  msg_p = itti_alloc_new_message (TASK_RRC_ENB_NB_IoT, S1AP_INITIAL_CONTEXT_SETUP_RESP);
...
//------------------------------------------------------------------------------
void
rrc_eNB_send_S1AP_UPLINK_NAS_NB_IoT(
  const protocol_ctxt_t*    const ctxt_pP,
  rrc_eNB_ue_context_NB_IoT_t*             const ue_context_pP,
...
//------------------------------------------------------------------------------
{
...
#else
  {
    ...
    ulInformationTransfer = &ul_dcch_msg->message.choice.c1.choice.ulInformationTransfer_r13;

    if (ulInformationTransfer->criticalExtensions.present == ULInformationTransfer_NB__criticalExtensions_PR_ulInformationTransfer_r13) {
      
      ULInformationTransfer_NB_r13_IEs_t *ulInformationTransferR13;

      ulInformationTransferR13 = &ulInformationTransfer->criticalExtensions.choice.ulInformationTransfer_r13;

      s1ap_eNB_new_data_request (mod_id, ue_index,
          ulInformationTransferR13->dedicatedInfoNAS_r13.buf,
          ulInformationTransferR13->dedicatedInfoNAS_r13.size);
...
//------------------------------------------------------------------------------
{
  UECapabilityInformation_NB_t *ueCapabilityInformation = &ul_dcch_msg->message.choice.c1.choice.ueCapabilityInformation_r13;
  /* 4096 is arbitrary, should be big enough */
  unsigned char buf[4096];
  unsigned char *buf2;
  UERadioAccessCapabilityInformation_NB_t rac;

  if (ueCapabilityInformation->criticalExtensions.present != UECapabilityInformation_NB__criticalExtensions_PR_ueCapabilityInformation_r13) {
    LOG_E(RRC, "[NB_IoT][eNB %d][UE %x] bad UE capabilities\n", ctxt_pP->module_id, ue_context_pP->ue_context.rnti);
    return;
  }

  asn_enc_rval_t ret = uper_encode_to_buffer(&asn_DEF_UECapabilityInformation_NB, ueCapabilityInformation, buf, 4096);
  if (ret.encoded == -1) abort();

  memset(&rac, 0, sizeof(UERadioAccessCapabilityInformation_NB_t));

  rac.criticalExtensions.present = UERadioAccessCapabilityInformation_NB__criticalExtensions_PR_c1;
  rac.criticalExtensions.choice.c1.present = UERadioAccessCapabilityInformation_NB__criticalExtensions__c1_PR_ueRadioAccessCapabilityInformation_r13;
  rac.criticalExtensions.choice.c1.choice.ueRadioAccessCapabilityInformation_r13.ue_RadioAccessCapabilityInfo_r13.buf = buf;
  rac.criticalExtensions.choice.c1.choice.ueRadioAccessCapabilityInformation_r13.ue_RadioAccessCapabilityInfo_r13.size = (ret.encoded+7)/8;
  rac.criticalExtensions.choice.c1.choice.ueRadioAccessCapabilityInformation_r13.nonCriticalExtension = NULL;

  /* 8192 is arbitrary, should be big enough */
  buf2 = malloc16(8192);
  if (buf2 == NULL) abort();
  ret = uper_encode_to_buffer(&asn_DEF_UERadioAccessCapabilityInformation_NB, &rac, buf2, 8192);
  if (ret.encoded == -1) abort();

  MessageDef *msg_p;

  msg_p = itti_alloc_new_message (TASK_RRC_ENB_NB_IoT, S1AP_UE_CAPABILITIES_IND);
...
//------------------------------------------------------------------------------
void
rrc_eNB_send_S1AP_NAS_FIRST_REQ_NB_IoT(
  const protocol_ctxt_t* const ctxt_pP,
  rrc_eNB_ue_context_NB_IoT_t*          const ue_context_pP,
...
{
  eNB_RRC_INST_NB_IoT *rrc=RC.nb_iot_rrc[ctxt_pP->module_id];
#if defined(ENABLE_ITTI)
  {
    MessageDef*         message_p         = NULL;
    rrc_ue_s1ap_ids_NB_IoT_t*  rrc_ue_s1ap_ids_p = NULL;
    hashtable_rc_t      h_rc;
    ...
    h_rc = hashtable_insert(RC.nb_iot_rrc[ctxt_pP->module_id]->initial_id2_s1ap_ids,
    ...
    rrcConnectionSetupComplete_NB->dedicatedInfoNAS_r13.buf;
    S1AP_NAS_FIRST_REQ (message_p).nas_pdu.length = rrcConnectionSetupComplete_NB->dedicatedInfoNAS_r13.size;
    ...
    /* Fill UE identities with available information */
    {
      ...
      if (ue_context_pP->ue_context.Initialue_identity_s_TMSI.presence) {
        /* Fill s-TMSI */
        UE_S_TMSI_NB_IoT* s_TMSI = &ue_context_pP->ue_context.Initialue_identity_s_TMSI;
      ...
      if (rrcConnectionSetupComplete_NB->registeredMME_r13 != NULL) {
        /* Fill GUMMEI */
        struct RegisteredMME *r_mme = rrcConnectionSetupComplete_NB->registeredMME_r13;
        //int selected_plmn_identity = rrcConnectionSetupComplete_NB->selectedPLMN_Identity;
...
#else
  {
    s1ap_eNB_new_data_request (
      ctxt_pP->module_id,
      ue_context_pP,
      rrcConnectionSetupComplete_NB->dedicatedInfoNAS_r13.
      buf,
      rrcConnectionSetupComplete_NB->dedicatedInfoNAS_r13.
      size);
  }
...
//------------------------------------------------------------------------------
int
rrc_eNB_process_S1AP_DOWNLINK_NAS_NB_IoT(
  MessageDef* msg_p,
  const char* msg_name,
  instance_t instance,
  mui_t* rrc_eNB_mui
...
//------------------------------------------------------------------------------
{
  ...
  ue_context_p = rrc_eNB_get_ue_context_from_s1ap_ids_NB_IoT(instance, ue_initial_id, eNB_ue_s1ap_id);
  ...
  if (ue_context_p == NULL) {
    ...
    msg_fail_p = itti_alloc_new_message (TASK_RRC_ENB_NB_IoT, S1AP_NAS_NON_DELIVERY_IND);
    ...
    srb_id = ue_context_p->ue_context.Srb1.Srb_info.Srb_id;
    ...
    // no primitive for this moment, we are going to use the old one
    /*rrc_data_req_NB_IoT (
		  &ctxt,
		  srb_id,
		  *rrc_eNB_mui++,
...
/*------------------------------------------------------------------------------*/
int rrc_eNB_process_S1AP_INITIAL_CONTEXT_SETUP_REQ_NB_IoT(MessageDef *msg_p, const char *msg_name, instance_t instance)
{
  uint16_t                        ue_initial_id;
  uint32_t                        eNB_ue_s1ap_id;
  ...
  if (ue_context_p == NULL) {
    ...
    msg_fail_p = itti_alloc_new_message (TASK_RRC_ENB_NB_IoT, S1AP_INITIAL_CONTEXT_SETUP_FAIL);
    ...
    process_eNB_security_key_NB_IoT (
      &ctxt,
      ue_context_p,
...

    {
      ...
      if (send_security_mode_command) {
        ...
        rrc_eNB_generate_UECapabilityEnquiry_NB_IoT (&ctxt, ue_context_p);
      }
    }
    return (0);
  }
...
/*------------------------------------------------------------------------------*/
int rrc_eNB_process_S1AP_UE_CTXT_MODIFICATION_REQ_NB_IoT(MessageDef *msg_p, const char *msg_name, instance_t instance)
{
  uint32_t eNB_ue_s1ap_id;
  struct rrc_eNB_ue_context_NB_IoT_s* ue_context_p = NULL;
  protocol_ctxt_t              ctxt;
  ...
  if (ue_context_p == NULL) {
    ...
    msg_fail_p = itti_alloc_new_message (TASK_RRC_ENB_NB_IoT, S1AP_UE_CTXT_MODIFICATION_FAIL);
    ...
    if (S1AP_UE_CTXT_MODIFICATION_REQ(msg_p).present & S1AP_UE_CONTEXT_MODIFICATION_UE_SECU_CAP) {
      if (rrc_eNB_process_security_NB_IoT (
            &ctxt,
            ue_context_p,
    ...
    /* Send the response */
    {
      ...
      msg_resp_p = itti_alloc_new_message(TASK_RRC_ENB_NB_IoT, S1AP_UE_CTXT_MODIFICATION_RESP);
...
/*------------------------------------------------------------------------------*/
int rrc_eNB_process_S1AP_UE_CONTEXT_RELEASE_REQ_NB_IoT (MessageDef *msg_p, const char *msg_name, instance_t instance)
{
  uint32_t eNB_ue_s1ap_id;
  rrc_eNB_ue_context_NB_IoT_t* ue_context_p = NULL;
  ...
  if (ue_context_p == NULL) {
    ...
    msg_fail_p = itti_alloc_new_message(TASK_RRC_ENB_NB_IoT, S1AP_UE_CONTEXT_RELEASE_RESP); /* TODO change message ID. */
    ...
    /* Send the response */
    {
      ...
      msg_resp_p = itti_alloc_new_message(TASK_RRC_ENB_NB_IoT, S1AP_UE_CONTEXT_RELEASE_RESP);
...
//------------------------------------------------------------------------------
void rrc_eNB_send_S1AP_UE_CONTEXT_RELEASE_REQ_NB_IoT (
  const module_id_t                        enb_mod_idP,
  const rrc_eNB_ue_context_NB_IoT_t*        const ue_context_pP,
...
//------------------------------------------------------------------------------
{
  if (ue_context_pP == NULL) {
    ...
    msg_context_release_req_p = itti_alloc_new_message(TASK_RRC_ENB_NB_IoT, S1AP_UE_CONTEXT_RELEASE_REQ);
...
int rrc_eNB_process_S1AP_UE_CONTEXT_RELEASE_COMMAND_NB_IoT (MessageDef *msg_p, const char *msg_name, instance_t instance)
{
  ...
  protocol_ctxt_t              ctxt;
  struct rrc_eNB_ue_context_NB_IoT_s *ue_context_p = NULL;
  struct rrc_ue_s1ap_ids_NB_IoT_s    *rrc_ue_s1ap_ids = NULL;
  ...
  if (ue_context_p == NULL) {
    ...
    msg_complete_p = itti_alloc_new_message(TASK_RRC_ENB_NB_IoT, S1AP_UE_CONTEXT_RELEASE_COMPLETE);
    S1AP_UE_CONTEXT_RELEASE_COMPLETE(msg_complete_p).eNB_ue_s1ap_id = eNB_ue_s1ap_id;
    itti_send_msg_to_task(TASK_S1AP, instance, msg_complete_p);

    rrc_ue_s1ap_ids = rrc_eNB_S1AP_get_ue_ids_NB_IoT(
    		RC.nb_iot_rrc[instance],
    ...
    PROTOCOL_CTXT_SET_BY_INSTANCE(&ctxt, instance, ENB_FLAG_YES, ue_context_p->ue_context.rnti, 0, 0);
    rrc_eNB_generate_RRCConnectionRelease_NB_IoT(&ctxt, ue_context_p);
    /*
          LOG_W(RRC,
    ...
    */
    {
      ...
      msg_delete_tunnels_p = itti_alloc_new_message(TASK_RRC_ENB_NB_IoT, GTPV1U_ENB_DELETE_TUNNEL_REQ);
...
int rrc_eNB_process_S1AP_E_RAB_SETUP_REQ_NB_IoT(MessageDef *msg_p, const char *msg_name, instance_t instance)
{
  uint16_t                        ue_initial_id;
  uint32_t                        eNB_ue_s1ap_id;
  gtpv1u_enb_create_tunnel_req_t  create_tunnel_req;
  ...
  if (ue_context_p == NULL) {
    ...
    msg_fail_p = itti_alloc_new_message (TASK_RRC_ENB_NB_IoT, S1AP_E_RAB_SETUP_REQUEST_FAIL);
...
int rrc_eNB_send_S1AP_E_RAB_SETUP_RESP_NB_IoT(const protocol_ctxt_t* const ctxt_pP,
				   rrc_eNB_ue_context_NB_IoT_t*          const ue_context_pP,
				   uint8_t xid ){
  ...
  msg_p = itti_alloc_new_message (TASK_RRC_ENB_NB_IoT, S1AP_E_RAB_SETUP_RESP);
