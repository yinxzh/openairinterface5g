...
void lte_idft_NB_IoT(NB_IoT_DL_FRAME_PARMS *frame_parms,uint32_t *z, uint16_t Msc_PUSCH)
{
  ...
  //  printf("Doing lte_idft for Msc_PUSCH %d\n",Msc_PUSCH);
   ...
   // Normal prefix
    z0 = z;
    z1 = z0+(frame_parms->LTE_N_RB_DL*12);
    z2 = z1+(frame_parms->LTE_N_RB_DL*12);
    //pilot
    z3 = z2+(2*frame_parms->LTE_N_RB_DL*12);
    z4 = z3+(frame_parms->LTE_N_RB_DL*12);
    z5 = z4+(frame_parms->LTE_N_RB_DL*12);

    z6 = z5+(frame_parms->LTE_N_RB_DL*12);
    z7 = z6+(frame_parms->LTE_N_RB_DL*12);
    z8 = z7+(frame_parms->LTE_N_RB_DL*12);
    //pilot
    z9 = z8+(2*frame_parms->LTE_N_RB_DL*12);
    z10 = z9+(frame_parms->LTE_N_RB_DL*12);
    // srs
    z11 = z10+(frame_parms->LTE_N_RB_DL*12);
...
{
#if defined(__x86_64__) || defined(__i386__)
  __m128i *rxF=(__m128i*)&rxdataF_comp[0][(symbol*frame_parms->LTE_N_RB_DL*12)];
  __m128i **llrp128 = (__m128i **)llrp;
#elif defined(__arm__)
  int16x8_t *rxF= (int16x8_t*)&rxdataF_comp[0][(symbol*frame_parms->LTE_N_RB_DL*12)];
  int16x8_t **llrp128 = (int16x8_t **)llrp;
#endif

  int i;

  //  printf("qpsk llr for symbol %d (pos %d), llr offset %d\n",symbol,(symbol*frame_parms->LTE_N_RB_DL*12),llr128U-(__m128i*)ulsch_llr);
...
#if defined(__x86_64__) || defined(__i386__)
    rxdataF_comp128_0   = (__m128i *)&rxdataF_comp[0][symbol*frame_parms->LTE_N_RB_DL*12];
    rxdataF_comp128_1   = (__m128i *)&rxdataF_comp[1][symbol*frame_parms->LTE_N_RB_DL*12];
    ul_ch_mag128_0      = (__m128i *)&ul_ch_mag[0][symbol*frame_parms->LTE_N_RB_DL*12];
    ul_ch_mag128_1      = (__m128i *)&ul_ch_mag[1][symbol*frame_parms->LTE_N_RB_DL*12];
    ul_ch_mag128_0b     = (__m128i *)&ul_ch_magb[0][symbol*frame_parms->LTE_N_RB_DL*12];
    ul_ch_mag128_1b     = (__m128i *)&ul_ch_magb[1][symbol*frame_parms->LTE_N_RB_DL*12];
...
#elif defined(__arm__)
    rxdataF_comp128_0   = (int16x8_t *)&rxdataF_comp[0][symbol*frame_parms->LTE_N_RB_DL*12];
    rxdataF_comp128_1   = (int16x8_t *)&rxdataF_comp[1][symbol*frame_parms->LTE_N_RB_DL*12];
    ul_ch_mag128_0      = (int16x8_t *)&ul_ch_mag[0][symbol*frame_parms->LTE_N_RB_DL*12];
    ul_ch_mag128_1      = (int16x8_t *)&ul_ch_mag[1][symbol*frame_parms->LTE_N_RB_DL*12];
    ul_ch_mag128_0b     = (int16x8_t *)&ul_ch_magb[0][symbol*frame_parms->LTE_N_RB_DL*12];
    ul_ch_mag128_1b     = (int16x8_t *)&ul_ch_magb[1][symbol*frame_parms->LTE_N_RB_DL*12];
...
//  uint16_t ul_sc_start; // subcarrier start index into UL RB 
  ...
  for (aarx=0; aarx<frame_parms->nb_antennas_rx; aarx++) {
    ...
    nb_rb1 = cmin(cmax((int)(frame_parms->LTE_N_RB_UL) - (int)(2*UL_RB_ID_NB_IoT),(int)0),(int)(2));    // 2 times no. RBs before the DC
                                 // 2 times no. RBs after the DC

    // rxF_ext   = &rxdataF_ext[aarx][(symbol*frame_parms->LTE_N_RB_UL*12)];
    ...
    if (nb_rb1) { // RB NB-IoT is in the first half
      ...
      for (n=0;n<12;n++){ // extract whole RB of 12 subcarriers
        ...
        rxdataF_ext[aarx][symbol*frame_parms->LTE_N_RB_UL*12 + n] = rxdataF[aarx][UL_RB_ID_NB_IoT*12 + frame_parms->first_carrier_offset + symbol*frame_parms->ofdm_symbol_size + n];
      ...
      for (n=0;n<12;n++){ // extract whole RB of 12 subcarriers
        // Note that FFT splits the RBs 
        // rxdataF_ext[aarx][symbol*frame_parms->LTE_N_RB_UL*12 + n] = rxdataF[aarx][6*(2*UL_RB_ID_NB_IoT - frame_parms->N_RB_UL) +  ul_sc_start + symbol*frame_parms->ofdm_symbol_size + n]; 
        rxdataF_ext[aarx][symbol*frame_parms->LTE_N_RB_UL*12 + n] = rxdataF[aarx][6*(2*UL_RB_ID_NB_IoT - frame_parms->LTE_N_RB_UL) + symbol*frame_parms->ofdm_symbol_size + n];

      
      }

      //#ifdef OFDMA_ULSCH
      //      rxF = &rxdataF[aarx][(1 + 6*(2*first_rb - frame_parms->LTE_N_RB_UL) + symbol*frame_parms->ofdm_symbol_size)*2];
      //#else
      // rxF = &rxdataF[aarx][(6*(2*first_rb - frame_parms->LTE_N_RB_UL) + symbol*frame_parms->ofdm_symbol_size)]; 
      // //#endif
      // memcpy(rxF_ext, rxF, nb_rb2*6*sizeof(int));
      // rxF_ext += nb_rb2*6;
    }
                                ...
                                uint8_t output_shift)
{
  ...
  for (aarx=0; aarx<frame_parms->nb_antennas_rx; aarx++) {
    ...
    ul_ch128          = (__m128i *)&ul_ch_estimates_ext[aarx][symbol*frame_parms->LTE_N_RB_DL*12];
    ul_ch_mag128      = (__m128i *)&ul_ch_mag[aarx][symbol*frame_parms->LTE_N_RB_DL*12];
    ul_ch_mag128b     = (__m128i *)&ul_ch_magb[aarx][symbol*frame_parms->LTE_N_RB_DL*12];
    rxdataF128        = (__m128i *)&rxdataF_ext[aarx][symbol*frame_parms->LTE_N_RB_DL*12];
    rxdataF_comp128   = (__m128i *)&rxdataF_comp[aarx][symbol*frame_parms->LTE_N_RB_DL*12];

#elif defined(__arm__)


    ul_ch128          = (int16x4_t *)&ul_ch_estimates_ext[aarx][symbol*frame_parms->LTE_N_RB_DL*12];
    ul_ch_mag128      = (int16x8_t *)&ul_ch_mag[aarx][symbol*frame_parms->LTE_N_RB_DL*12];
    ul_ch_mag128b     = (int16x8_t *)&ul_ch_magb[aarx][symbol*frame_parms->LTE_N_RB_DL*12];
    rxdataF128        = (int16x4_t *)&rxdataF_ext[aarx][symbol*frame_parms->LTE_N_RB_DL*12];
    rxdataF_comp128   = (int16x8_t *)&rxdataF_comp[aarx][symbol*frame_parms->LTE_N_RB_DL*12];
                            ...
                            uint8_t symbol)
{
  ...
  rxdataF_comp32   = (int32_t *)&rxdataF_comp[0][symbol*frame_parms->LTE_N_RB_DL*12]; 
  if (Nsc_RU != 12){
                                  ...
                                  uint8_t Qm)
{
  ...
  ul_sc_start = get_UL_sc_start_NB_IoT(I_sc); // NB-IoT: get the used subcarrier in RB
  rxdataF_comp16   = (int16_t *)&rxdataF_comp[0][symbol*frame_parms->LTE_N_RB_DL*12 + ul_sc_start]; 
...
//  rx_power_correction = 1;
  ...
  for (l=0; l<frame_parms->symbols_per_tti; l++) { 
    ...
    ulsch_extract_rbs_single_NB_IoT(common_vars->rxdataF,
  ...
  T(T_ENB_PHY_PUSCH_IQ, T_INT(eNB_id), T_INT(UE_id), T_INT(proc->frame_rx),
    ...
    T_INT(frame_parms->LTE_N_RB_UL), T_INT(frame_parms->symbols_per_tti),
    T_BUFFER(pusch_vars->rxdataF_comp[eNB_id][0],
             2 * /* ulsch[UE_id]->harq_processes[harq_pid]->nb_rb */ frame_parms->LTE_N_RB_UL *12*frame_parms->symbols_per_tti*2));
