...
int rx_nsss_NB_IoT(PHY_VARS_UE_NB_IoT *ue,int32_t *tot_metric)
{
	...
	for (l=0;l<11;l++){
		nsss_extract_NB_IoT(ue,frame_parms,(int32_t **)nsss_ext,l);  
	} 
						...
						int l)
{
	...
	for (aarx=0; aarx<frame_parms->nb_antennas_rx; aarx++) {
		...
		if (UL_RB_ID_NB_IoT <= (frame_parms->LTE_N_RB_DL>>1)) { // NB-IoT RB is in the first half 
          ...
          nsss_rxF       = &rxdataF[aarx][(1 + 6*(2*UL_RB_ID_NB_IoT - frame_parms->LTE_N_RB_DL) + ((l+first_symbol_offset)*(frame_parms->ofdm_symbol_size)))];
          //dl_ch0++;
        } 
